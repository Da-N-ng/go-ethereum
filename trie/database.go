...
// hashDatabase is an intermediate write layer between the trie data structures and
...
func openHashDatabase(diskdb ethdb.Database, cleans *fastcache.Cache) *hashDatabase {
	return &hashDatabase{
		diskdb: diskdb,
		cleans: cleans,
...
func (db *hashDatabase) insert(hash common.Hash, size int, node node) {
...
func (db *hashDatabase) node(hash common.Hash) *memoryNode {
	// Retrieve the node from the clean cache if available
	if db.cleans != nil {
		if enc := db.cleans.Get(nil, hash[:]); enc != nil {
			memcacheCleanHitMeter.Mark(1)
			memcacheCleanReadMeter.Mark(int64(len(enc)))
			return &memoryNode{node: rawNode(enc), hash: hash, size: uint16(len(enc))}
		}
	}
	...
	if dirty != nil {
		...
		return &memoryNode{node: dirty.node, size: dirty.size, hash: hash}
	}
...
func (db *hashDatabase) Node(hash common.Hash) ([]byte, error) {
...
// states in test code.
func (db *hashDatabase) Nodes() []common.Hash {
	db.lock.RLock()
...
func (db *hashDatabase) Reference(root common.Hash) {
	db.lock.Lock()
...
func (db *hashDatabase) reference(child common.Hash, parent common.Hash) {
...
func (db *hashDatabase) Dereference(root common.Hash) {
...
func (db *hashDatabase) dereference(child common.Hash, parent common.Hash) {
...
func (db *hashDatabase) Cap(limit common.StorageSize) error {
...
// Note, this method is a non-synchronized mutator. It is unsafe to call this
// concurrently with other mutators.
func (db *hashDatabase) Commit(node common.Hash) error {
	// Create a database batch to flush persistent data out. It is important that
	// outside code doesn't see an inconsistent state (referenced data removed from
	// memory cache during commit but not yet in persistent storage). This is ensured
	// by only uncaching existing data when the database write finalizes.
	...
	log.Debug("Persisted trie from memory database", "nodes", nodes-len(db.dirties)+int(db.flushnodes), "size", storage-db.dirtiesSize+db.flushsize, "time", time.Since(start)+db.flushtime,
		"gcnodes", db.gcnodes, "gcsize", db.gcsize, "gctime", db.gctime, "livenodes", len(db.dirties), "livesize", db.dirtiesSize)
...
func (db *hashDatabase) commit(hash common.Hash, batch ethdb.Batch, uncacher *cleaner) error {
...
func (db *hashDatabase) Size() common.StorageSize {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
// Hash database is only regarded as empty if genesis state is
// missing.
...
func (reader *hashReader) Node(path []byte, hash common.Hash) (*memoryNode, error) {
...
func (db *hashDatabase) Update(_ common.Hash, _ common.Hash, nodes *NodeSet) error {
	db.lock.Lock()
	defer db.lock.Unlock()

	lookup := make(map[string]*leaf)
	for _, n := range nodes.leaves {
		lookup[string(n.parent)] = n
	}
	for _, key := range nodes.keys {
		// Insert dirty nodes into the database in order. Insertion order
		// must be from bottom to top.
		n, ok := nodes.nodes[string(key)]
		if !ok {
			owner, path := DecodeStorageKey(key)
			return fmt.Errorf("missing node %x %v", owner, path)
		}
		db.insert(n.hash, int(n.size), n.node)

		// Try to link the account trie and storage trie if current node
		// points to a leaf node.
		if leaf, ok := lookup[string(key)]; ok {
			var account types.StateAccount
			if err := rlp.DecodeBytes(leaf.val, &account); err != nil {
				continue // the leaf of storage trie
			}
			if account.Root != emptyRoot {
				db.reference(account.Root, leaf.parentHash)
			}
		}
	}
	return nil
...
func (scheme *hashScheme) HasTrieNode(db ethdb.KeyValueReader, key []byte, hash common.Hash) bool {
	return rawdb.HasLegacyTrieNode(db, hash)
}

// ReadTrieNode retrieves the trie node from database with the provided node key
// and associated node hash.
func (scheme *hashScheme) ReadTrieNode(db ethdb.KeyValueReader, key []byte, hash common.Hash) []byte {
	return rawdb.ReadLegacyTrieNode(db, hash)
}

// WriteTrieNode writes the trie node into database with the provided node key
// and associated node hash.
func (scheme *hashScheme) WriteTrieNode(db ethdb.KeyValueWriter, key []byte, hash common.Hash, node []byte) {
	rawdb.WriteLegacyTrieNode(db, hash, node)
}

// DeleteTrieNode deletes the trie node from database with the provided node key
// and associated node hash.
func (scheme *hashScheme) DeleteTrieNode(db ethdb.KeyValueWriter, key []byte, hash common.Hash) {

...
func (db *snapDatabase) loadJournal(disk ethdb.Database, diskRoot common.Hash, cleans *fastcache.Cache) (snapshot, error) {
	...
	base, err := db.loadDiskLayer(r, cleans, disk)
	if err != nil {
		return nil, err
	}
...
func (db *snapDatabase) loadSnapshot(diskdb ethdb.Database, cleans *fastcache.Cache) snapshot {
	...
	snap, err := db.loadJournal(diskdb, root, cleans)
	if err == nil {
		return snap
	}
	...
	return newDiskLayer(root, rawdb.ReadReverseDiffHead(diskdb), cleans, newDiskcache(db.dirtySize, nil, 0), diskdb)
}

// loadDiskLayer reads the binary blob from the snapshot journal, reconstructing a new
// disk layer on it.
func (db *snapDatabase) loadDiskLayer(r *rlp.Stream, clean *fastcache.Cache, disk ethdb.Database) (snapshot, error) {
	...
	base := newDiskLayer(root, diffid, clean, newDiskcache(db.dirtySize, nodes, diffid-diskid), disk)
	return base, nil
}

...
type snapshot interface {
	...
	Update(blockRoot common.Hash, id uint64, nodes map[string]*nodeWithPreValue) *diffLayer
...
// nodeWithPreValue wraps the memoryNode with the previous node value.
...
func (n *nodeWithPreValue) unwrap() *memoryNode {
	return n.memoryNode
}

// memorySize returns the total memory size used by this node. It overloads
// the function in memoryNode by counting the size of previous value.
func (n *nodeWithPreValue) memorySize(key int) int {
	return n.memoryNode.memorySize(key) + len(n.pre)
}
...
func (db *snapDatabase) Update(root common.Hash, parentRoot common.Hash, nodes *NodeSet) error {
	...
	if err := db.tree.add(root, parentRoot, nodes.nodes); err != nil {
		return err
	}
	var limit uint64
	if db.config != nil {
...
// Commit traverses downwards the snapshot tree from a head block hash until the
// number of allowed layers are crossed. All layers beyond the permitted number
// are flattened downwards. It can be used alone and mostly for test purposes.
func (db *snapDatabase) Commit(root common.Hash) error {
	// Hold the lock to prevent concurrent mutations.
	db.lock.Lock()
	defer db.lock.Unlock()
...
// Cap iteratively flushes bottom-most dirty nodes into disk until the total
// memory usage goes below the given threshold. It's not implemented yet.
func (db *snapDatabase) Cap(limit common.StorageSize) error {
	return nil
}
...
func (db *snapDatabase) Reset(root common.Hash) error {
	...
	if root == emptyRoot {
		...
		// Ensure the specified state is present in disk.
...
func (scheme *pathScheme) HasTrieNode(db ethdb.KeyValueReader, key []byte, hash common.Hash) bool {
	return rawdb.HasTrieNode(db, key, hash)
}

// ReadTrieNode retrieves the trie node from database with the provided node key
// and associated node hash.
func (scheme *pathScheme) ReadTrieNode(db ethdb.KeyValueReader, key []byte, hash common.Hash) []byte {
	blob, h := rawdb.ReadTrieNode(db, key)
	if h != hash {
		return nil
	}
	return blob
...
// and associated node hash.
func (scheme *pathScheme) WriteTrieNode(db ethdb.KeyValueWriter, key []byte, hash common.Hash, node []byte) {
	rawdb.WriteTrieNode(db, key, node)
}

// DeleteTrieNode deletes the trie node from database with the provided node key
// and associated node hash.
func (scheme *pathScheme) DeleteTrieNode(db ethdb.KeyValueWriter, key []byte, hash common.Hash) {
	rawdb.DeleteTrieNode(db, key)
}

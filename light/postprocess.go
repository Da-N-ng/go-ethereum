...
func NewChtIndexer(db ethdb.Database, odr OdrBackend, size, confirms uint64, disablePruning bool) *core.ChainIndexer {
	...
	backend := &ChtIndexerBackend{
		...
		odr:            odr,
		trieTable:      trieTable,
		triedb:         trie.NewDatabase(trieTable, &trie.Config{Cache: 1, Scheme: trie.HashScheme}), // Use a tiny cache only to keep memory down
		sectionSize:    size,
...
func (c *ChtIndexerBackend) Commit() error {
	root, nodes, err := c.trie.Commit()
	if err != nil {
		return err
	}
	if err := c.triedb.Update(common.Hash{}, common.Hash{}, nodes); err != nil {
		return err // roots are not used, ok to pass empty ones
	}
	...
	if !c.disablePruning {
		...
		nodes.ForEach(func(_ []byte, blob []byte) {
			if len(blob) == 0 {
				return
			}
			hashes[crypto.Keccak256Hash(blob)] = struct{}{}
		})
		for it.Next() {
...
func (b *BloomTrieIndexerBackend) Commit() error {
	...
	root, nodes, err := b.trie.Commit()
	if err != nil {
		return err
	}
	err = b.triedb.Update(common.Hash{}, common.Hash{}, nodes)
	if err != nil {
		return err
	}
	// Pruning historical trie nodes if necessary.
